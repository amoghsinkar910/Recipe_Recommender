...
while True:
    ...
    toplist = sorted(simlist, reverse=True) [:1]

    displist = []

    for tup in toplist:
        ind = tup[1]
        recipe_name = df1.iloc[ind, 0]

        ind2 = jsondata[jsondata['title'] == recipe_name].index[0] #time-consuming
        print("\nIndex: ", ind2) # Prints index from json
        dish_title = jsondata.at[ind2, 'title']
        print('Recipe Name: ', dish_title)
        dish_match = round(tup[0]*100, 2)
        print('Match: ', dish_match)
        dish_sodium = jsondata.at[ind2, 'sodium']
